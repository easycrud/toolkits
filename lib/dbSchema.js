function json2mysql(obj, opts = {
  overwrite: false,
  engine: 'InnoDB',
  autoIncrement: 0,
}) {
  const {overwrite, engine} = opts;
  ...
  if (overwrite) {
    createStat = `DROP TABLE IF EXISTS \`${tableName}\`;${createStat}`;
  }
  ...
  const tableStat = `ENGINE=${engine}${autoIncrement} DEFAULT CHARSET=utf8`;
